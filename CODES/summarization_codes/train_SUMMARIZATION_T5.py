# Fine-tuning Flan-T5 for explanation generation on PubHealth
# Input: claim + main_text → Output: explanation
...
MODEL_SHORT = "flan-t5-explanation"
EPOCHS      = 5
BATCH_SIZE  = 8
MAX_INPUT   = 512
MAX_TARGET  = 128
...
RESULTS_PATH = "/home/lnlpG08/nlp/CODES/results/flan_t5_explanation/"
...
# 2. TOKENIZACIÓN
# Flan-T5 benefits from explicit instruction prompts

tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME)

def tokenize(examples):
    inputs = [
        f"Generate a comprehensive fact-checking explanation using multiple sentences. "
        f"Explain the evidence, medical reasoning, limitations, and conclusion in detail.\n\n"
        f"Claim: {c}\n\nArticle: {a}"
...
training_args = Seq2SeqTrainingArguments(
    ...
    warmup_ratio=0.1,
    weight_decay=0.01,
    fp16=False,              # T5 has issues with fp16, use bf16 if available
...
results_file = os.path.join(RESULTS_PATH, "predictions_flan_t5.csv")
