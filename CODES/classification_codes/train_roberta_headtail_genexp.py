...
# RoBERTa head+tail usando la explicación generada por RAG-BART en vez del main_text
# Input: claim + generated_explanation → Output: true / false / mixture / unproven
# Mejor modelo para: TRUE, FALSE, MIXTURE
...
MODEL_SHORT = "roberta_headtail_genexp"
EPOCHS      = 3
BATCH_SIZE  = 16
MAX_LENGTH  = 512
 
DATA_PATH    = "/home/lnlpG08/nlp/DATA/"
RESULTS_PATH = "/home/lnlpG08/nlp/RESULTS/roberta_headtail_genexp"
...
print(f"  Input: claim + generated_explanation (RAG-BART)")
 
# ─────────────────────────────────────────────────────────────────────────────
# CARGAR DATOS
# Train y dev → CSVs con explicaciones generadas
# Test        → predictions_rag_bart.csv (ya tenía generated_explanation)
# ─────────────────────────────────────────────────────────────────────────────
df_train = pd.read_csv(DATA_PATH + "train_genexp.csv")
df_dev   = pd.read_csv(DATA_PATH + "dev_genexp.csv")
df_test  = pd.read_csv("/home/lnlpG08/nlp/RESULTS/rag_bart_explanation/predictions_rag_bart.csv")
 
print(f"\nTrain: {len(df_train)} | Dev: {len(df_dev)} | Test: {len(df_test)}")
 
for df, nombre in [(df_train, "train"), (df_dev, "dev"), (df_test, "test")]:
    antes = len(df)
    df.dropna(subset=["claim", "generated_explanation"], inplace=True)
    if len(df) < antes:
...
# ─────────────────────────────────────────────────────────────────────────────
# TOKENIZACIÓN HEAD+TAIL
# Igual que el original pero usando generated_explanation en vez de main_text
# ─────────────────────────────────────────────────────────────────────────────
...
def tokenize_headtail(examples):
    ...
    for claim, genexp in zip(examples["claim"], examples["generated_explanation"]):
        claim_tokens  = tokenizer(claim, add_special_tokens=False)["input_ids"]
        available     = MAX_LENGTH - len(claim_tokens) - 3
        half          = available // 2
        genexp_tokens = tokenizer(str(genexp), add_special_tokens=False)["input_ids"]
 
        if len(genexp_tokens) <= available:
            selected = genexp_tokens
        else:
            selected = genexp_tokens[:half] + genexp_tokens[-half:]
...
# CREAR DATASETS
# ─────────────────────────────────────────────────────────────────────────────
train_dataset = Dataset.from_pandas(df_train[["claim", "generated_explanation", "label"]].reset_index(drop=True))
dev_dataset   = Dataset.from_pandas(df_dev[["claim", "generated_explanation", "label"]].reset_index(drop=True))
test_dataset  = Dataset.from_pandas(df_test[["claim", "generated_explanation", "label"]].reset_index(drop=True))
