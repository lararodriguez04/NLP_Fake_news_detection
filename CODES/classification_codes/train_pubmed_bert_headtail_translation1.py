...
# Input: claim + main_text con estrategia head+tail → Output: true / false / mixture / unproven
...
MODEL_SHORT = "biomed_bert_headtail"
EPOCHS = 3
BATCH_SIZE = 16
MAX_LENGTH = 512

DATA_PATH = "/home/lnlpG08/nlp/DATA/"
RESULTS_PATH = "/home/lnlpG08/nlp/CODES/results/biomed_bert_headtail"
...
df_train.rename(columns={"claim": "text_1"}, inplace=True)
df_dev.rename(columns={"claim": "text_1"}, inplace=True)
df_test.rename(columns={"claim": "text_1"}, inplace=True)
...
def tokenize_headtail(examples):
    ...
    for text_1, main_text in zip(examples["text_1"], examples["main_text"]):
        claim_tokens = tokenizer(text_1, add_special_tokens=False)["input_ids"]
        ...
        # BERT necesita token_type_ids: 0 para claim, 1 para main_text
        token_type_ids = [0] * (len(claim_tokens) + 2) + [1] * (len(selected) + 1)
    ...
    return {"input_ids": result_input_ids, "attention_mask": result_attention, "token_type_ids": result_token_type_ids}
